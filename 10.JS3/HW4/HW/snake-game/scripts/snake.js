...
var snakeGame = (function() {
  ...
  var Snake = MovingGameObject.extend({
    ...
    die: function() {
      ...
      if (this.dieHandlers) {
        /*for (var i = 0; i < this.dieHandlers.length; i++) {
          handler = this.dieHandlers[i];
          handler(result);
        };*/
    ...
    /*************************************/
    /* Implement the snake biting itself */
    /*************************************/
    intersectsS: function(other) {
      if (other instanceof SnakeHeadPiece) {
        return false;
      }

      if (other === this.pieces[1]) {
        return false;
      }
  ...
  /* TASK 07 */
  /*************************************/
  var MovingFood = MovingGameObject.extend({
    init: function(position, size, speed, direction) {
      this._super(position, size, foodFillColor, foodStrokeColor, speed, direction);
    },
    move: function() {
      this.position.x += directions[this.direction].x * this.speed;
      this.position.y += directions[this.direction].y * this.speed;
  ...
  var SnakeObstacle = Snake.extend({
    init: function(position, size, speed, direction) {
      this._super(position, speed, direction);
      var piece;
      var piecePosition;
      this.head = new SnakeObstacleHeadPiece(position, size, speed, direction);
      this.pieces = [this.head];
      for (var i = 1; i < defaultSnakeSize; i += 1) {
        piecePosition = {
          x: position.x - (directions[direction].x * size) * i,
          y: position.y - (directions[direction].y * size) * i,
        }
        piece = new SnakeObstaclePiece(piecePosition, size, speed, direction);
        this.pieces.push(piece);
      }
    },
    addDieHandler: function(){},
    die: function(){}
  });

  var SnakeObstaclePiece = SnakePiece.extend({
    init: function(position, size, speed, direction) {
      this._super(position, size, speed, direction);
      this.fcolor = obstacleFillColor;
      this.scolor = obstacleStrokeColor;
    }
  });

  var SnakeObstacleHeadPiece = SnakeObstaclePiece.extend({
    init: function(position, size, speed, direction) {
      this._super(position, size, speed, direction);
    }
  });
  ...
  var SnakeGame = Class.create({
    ...
    initGameObjects: function() {
      ...
      for (var i = 0; i < this.snake.pieces.length; i++) {
        this.gameObjects.push(this.snake.pieces[i]);
      };

      this.attachHandlerSnake();
    },
    ...
    /* BUG FIX */
    /***********************************************/
    /* I have extracted the setting of the die handler for the snake in a separate method that I call every time in the initGameObjects method*/
    attachHandlerSnake: function(){
      var self = this;
    ...
    /* TASK 06*/
    /***********************************************/
    /* I have extracted the keepInFrame method to be more easily testable*/
    keepInFrame: function(){
      var i;
    ...
    checkCollisions: function() {
      var i;
      // Add the counter numberOfCollisions for more easy testing
      var numberOfCollisions = 0;
      for (var i = 0; i < this.gameObjects.length; i += 1) {
        var gameObject = this.gameObjects[i];
        if (!(gameObject instanceof Snake)) {
          var colliding = this.snake.intersectsS(gameObject);
          if (colliding) {
            numberOfCollisions++;
            ...
            if (gameObject instanceof Food) { 
              ...
              /* TASK 08*/
              /***********************************************/
              if (this.piecesOfFoodEaten % 5 == 0){
                this.changeGameSpeed(this.timeInterval * 0.80);
              }
            }
          }          
        }
    ...
    /* TASK 08*/
    /***********************************************/
    changeGameSpeed: function(newSpeed){ 
      this.timeInterval = newSpeed;
      var self = this;
      if (this.snakeTimer) {
        clearInterval(this.snakeTimer);
        this.snakeTimer = setInterval(function() {
...
return {
  ...
  SnakeObstacle: SnakeObstacle,
  MovingFood: MovingFood,
  MovingObstacle: MovingObstacle,
  directions : directions,
  defaultSnakePieceSize : defaultSnakePieceSize,
};
}());