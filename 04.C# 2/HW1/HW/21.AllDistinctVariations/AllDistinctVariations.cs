...
class AllDistinctVariations
{
    static List<List<int>> GetAllVariations(List<int> elementsList, int lengthSubset)
    {
        ...
        if (lengthSubset > 0)
        {
            for (int i = 0; i <= elementsList.Count - lengthSubset; i++)
            {
                List<int> elementsSubList = new List<int>();
                elementsSubList.AddRange(elementsList.GetRange(i + 1, elementsList.Count - i - 1));
